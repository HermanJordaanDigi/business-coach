...
from src.db_helpers import get_all_sales, get_sales_by_date_range, get_table_stats
...
def main():
    ...
    # Header
    st.markdown(
        '<div class="main-header"> Business Coaching Analytics Dashboard</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        with col1:
            start_date = st.date_input(
                "Start Date", value=datetime(2025, 1, 1), key="start_date"
            )
        with col2:
            end_date = st.date_input(
                "End Date", value=datetime(2025, 11, 30), key="end_date"
            )
    ...
    with col3:
        ...
        collection_rate = (
            (cash_collected / total_revenue * 100) if total_revenue > 0 else 0
        )
        st.metric(
    ...
    with col4:
        upsell_rate = (
            (df_filtered["upsell"].sum() / len(df_filtered) * 100)
    ...
    with col1:
        ...
        revenue_by_month = df_filtered.groupby(
    ...
    with col2:
        ...
        product_revenue = (
            df_filtered.groupby("product")["revenue"].sum().sort_values(ascending=False)
        )
    ...
    with col2:
        ...
        country_revenue = (
            df_filtered.groupby("country")["revenue"].sum().sort_values(ascending=False)
        )
